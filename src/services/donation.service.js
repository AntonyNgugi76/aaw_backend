...
const getDonationsByPantry = async (pantryId, limit, skip) => {
  const donations = Donation.find({ dropOffPantry: pantryId }).populate(
    "user category dropOffPantry pickupAssignedPantry matchedRequest"
...
const getDonationsByCategory = async (categoryId, limit , skip) => {
  const donations =  Donation.find({ category: categoryId }).populate(
    "user category dropOffPantry pickupAssignedPantry matchedRequest"
  ).limit(limit).skip(skip);
  return{
    donations,
...
const getAllDonations = async ({ limit, skip, status }) => {
  let donations;
  let count;

  if (status === "all" || !status) {
    donations = await Donation.find().populate(
      "user category dropOffPantry pickupAssignedPantry matchedRequest"
    ).limit(limit).skip(skip);
    count = await Donation.countDocuments();
  } else {
    donations = await Donation.find({ status }).populate(
      "user category dropOffPantry pickupAssignedPantry matchedRequest"
    ).limit(limit).skip(skip);
    count = await Donation.countDocuments({ status });
  }
